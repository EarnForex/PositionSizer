...
input string TradeHotKey = "T"; // TradeHotKey: Case-insensitive hotkey. Supports Ctrl, Shift.
...
int OnInit()
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        if (StringSubstr(name, 0, 15) == "PS_ChartChange_")
        {
            ...
            if (prev_symbol != Symbol()) // Symbol changed.
            {
                ...
                // Check if price scale reset needed - will be actually checked in OnTimer().
                PrevChartWidth = (int)GlobalVariableGet(name);
    ...
    if (!ExtDialog.Create(0, Symbol() + " Position Sizer (ver. " + Version + ")", 0, DefaultPanelPositionX, DefaultPanelPositionY)) return INIT_FAILED;
...
void OnDeinit(const int reason)
{
    ...
    // Remember that via a Global Variable and then detect on consequent initialization.
    if (reason == REASON_CHARTCHANGE)
    {
        GlobalVariableSet("PS_ChartChange_" + Symbol(), (double)ChartGetInteger(ChartID(), CHART_WIDTH_IN_PIXELS)); // Chart width is used to detect going from wide-quote symbol to a narrower one.
    }
